...
try:
    ...
    logging.error(f"Failed to initialize RAG/Vector Search: {e}")
    RAG_MODEL = None
    FAISS_INDEX = None
...
def evaluate_outputs(original_output: str, refined_output: str, user_prompt: str) -> dict:
    """
    ...
    **Response B (from refined prompt):**
    ...
    try:
        ...
        # Using response_mime_type to enforce JSON output
        ...
        logging.error(f"Failed to parse JSON from evaluation response: {e}\nResponse text: {response.text}")
        return {"score_A": 0, "score_B": 0}
    except Exception as e:
